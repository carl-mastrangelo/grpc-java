...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
  ...
  private class ClientStreamListenerImpl implements ClientStreamListener {
    ...
    public void headersRead(final Metadata headers) {
      PerfMark.startTask("ClientCall.headersRead", tag);
    ...
    public void messagesAvailable(final MessageProducer producer) {
      PerfMark.startTask("ClientCall.messagesAvailable", tag);
      final Link link = PerfMark.link();
      ...
      PerfMark.stopTask("ClientCall.messagesAvailable", tag);
    }
    ...
    public void closed(Status status, RpcProgress rpcProgress, Metadata trailers) {
      PerfMark.startTask("ClientCall.closed", tag);
      final Link link = PerfMark.link();
      ...
      final class StreamClosed extends ContextRunnable {
        ...
        public final void runInContext() {
          ...
          PerfMark.startTask("ClientCall$Listener.closed", tag);
          link.link();
          try {
            close(savedStatus, savedTrailers);
          } finally {
            PerfMark.stopTask("ClientCall$Listener.closed", tag);
          }
        }
      }

      callExecutor.execute(new StreamClosed());
      PerfMark.stopTask("ClientCall.closed", tag);
    }

    @Override
    public void onReady() {
      PerfMark.startTask("ClientCall.onReady", tag);
      final Link link = PerfMark.link();
      ...
      PerfMark.stopTask("ClientCall.onReady", tag);
    }
  }
}
